...
/**
 * Model that blends a join log probability model and metadata training for online training and prediction on an exponential weighted predictor
 * @author Andres M
 *
 */
public  class LogProbabilityMetadataModelPredictor implements UserModelTrainerPredictor {
	
	
	
	private static final int NUM_PREDICTORS = 2;
	private FactorUserItemRepresentation modelRepresentation;
	private LogCombinationPredictor baseModel;
	private MetadataPredictor metadataModel;
	private double lossNormalization;
	
	public LogProbabilityMetadataModelPredictor(LogCombinationPredictor baseModel, MetadataPredictor metadataModel){
		this.baseModel= baseModel;
		this.metadataModel= metadataModel;
	}
	public LogProbabilityMetadataModelPredictor(FactorUserItemRepresentation representation){
...
public  Prediction calculatePrediction(UserTrainEvent event, int minTrains) throws TasteException{
	...
	public Vector calculatehyperParamsUpdate(UserTrainEvent event,Vector itemVector,
		...
		try{
			Prediction prediction1Pr=baseModel.calculatePrediction(event, 0);
			if(!prediction1Pr.isNoPrediction()){
				prediction1=prediction1Pr.getPredictionValue();
			}else{
	...
	public String toString(){
		return "LogProbabilityMetadataModelPredictor ("+baseModel.toString()+","+metadataModel.toString()+")" ;
	}
