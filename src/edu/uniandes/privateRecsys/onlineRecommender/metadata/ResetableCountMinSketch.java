 ...
 * 
 * Extention of com.clearspring.analytics.stream.frequency.CountMinSketch
 */
...
public class ResetableCountMinSketch  implements IFrequency{
    ...
    private ResetableCountMinSketch()
    {
    }

    public ResetableCountMinSketch(int depth, int width, long[]hashA)
    {
        this.depth = depth;
        this.width = width;
        this.eps = 2.0 / width;
        this.confidence = 1 - 1 / Math.pow(2, depth);
        initTablesWith(depth, width, hashA);
    }

    public ResetableCountMinSketch(double epsOfTotalCount, double confidence, long[]hashA)
    {
        // 2/w = eps ; w = 2/eps
    ...
    private ResetableCountMinSketch(int depth, int width, int size, long[] hashA, int[][] table)
    {
        this.depth = depth;
        this.width = width;
        this.eps   = 2.0 / width;
    ...
    private void initTablesWith(int depth, int width, long[]hashA)
    {
    ...
    public int hash(long item, int i)
    {
     ...
     */
    public static ResetableCountMinSketch merge(ResetableCountMinSketch... estimators) throws CMSMergeException
    {
    	ResetableCountMinSketch merged = null;
        if (estimators != null && estimators.length > 0)
        {
            ...
            for (ResetableCountMinSketch estimator : estimators)
            {
            ...
            merged = new ResetableCountMinSketch(depth, width, size, hashA, table);
        }

        return merged;
    }

    public static byte[] serialize(ResetableCountMinSketch sketch)
    {
    ...
    public static ResetableCountMinSketch deserialize(byte[] data)
    {
        ByteArrayInputStream bis = new ByteArrayInputStream(data);
        DataInputStream s = new DataInputStream(bis);
        try
        {
        	ResetableCountMinSketch sketch = new ResetableCountMinSketch();
            ...
            sketch.table = new int[sketch.depth][sketch.width];
            for (int i = 0; i < sketch.depth; ++i)
            {
                sketch.hashA[i] = s.readLong();
                for (int j = 0; j < sketch.width; ++j)
                {
                    sketch.table[i][j] = s.readInt();
                }
            }
            return sketch;
        }
